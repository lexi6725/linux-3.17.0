...
struct s3c_ts_mach_info s3c_ts_default_cfg  = {
                .delay = 		5000,//10000,
                .presc = 		49,
...
int X6410_adc_acquire_io(void) {
        ...
        if (!ret) {
                ...
                pr_info("forlinx debug****X6410_adc_acquire_io();.\n");
...
void X6410_adc_release_io(void) {
        ...
        pr_info("forlinx debug*****X6410_adc_release_io();.\n");
...
static void touch_timer_fire(unsigned long data)
{
	...
	data1 = readl(ts_base+S3C_ADCDAT1);
		...
		writel(WAIT4INT(0), ts_base+S3C_ADCTSC);
                ...
                if (ADC_locked4TS()) {
                        s3c_ts_adc_unlock();
                        pr_info("forlinx debug*****s3c_ts_adc_unlock();.\n");
                }
#endif
	}
...
static irqreturn_t stylus_updown(int irqno, void *param)
{
	...
	unsigned long data1;
        ...
        if (!ADC_locked4TS()) {
                ...
                pr_info("forlinx debug***** s3c_ts_adc_lock(LOCK_TS);.\n");
        }
#endif
...
static int s3c_ts_probe(struct platform_device *pdev)
{
	...
	ts_clock = devm_clk_get(&pdev->dev, "adc");
	if (IS_ERR(ts_clock)) {
	...
	if (ts_irq == NULL) {
        ...
        ret = request_irq(ts_irq->start, stylus_action, 0 | IRQF_SHARED,
		 "s3c_action", ts);
	if (ret != 0) {
...
static char banner[] = KERN_INFO "S3C Touchscreen driver, (c) 2008 Samsung Electronics\n";
